    ...
    fn test_scan_tool_area_3() {
        ...
        impl Runnable for TestRobot {
            fn process_tick(&mut self, world: &mut World) {
                ...
                // expected results from the scan tool
                let expected_results = [
                    None,
                    None,
                    Some((MapCoordinate::new(2, 2), 1)),
                    Some((MapCoordinate::new(2, 2), 1)),
                    Some((MapCoordinate::new(2, 2), 1)),
                    Some((MapCoordinate::new(2, 2), 1)),
                    Some((MapCoordinate::new(2, 2), 1)),
                ...
                for (phase, dir) in directions.iter().enumerate() {
                    ...
                    match result {
                        Ok(content) => {
                            assert_eq!(expected_results[phase], content);
                        }
                        Err(_) => panic!(),
                    }
